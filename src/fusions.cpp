			    ...
			    bool update_stat)
{
  ...
  for(size_t i = 0; i < new_fusions.size(); ++i)
      ...
      for (; fusion_pos < cigars.size(); ++fusion_pos)
	{
	  ...
	  if (opcode == FUSION_FF || opcode == FUSION_FR || opcode == FUSION_RF)
	    break;
	}
	...
	{
	  assert(fusion.refid != 0xFFFFFFFF);
      ...
      if (update_stat)
	{
	  ...
	  if (fusion.dir == FUSION_FF)
	    {
	      FusionSet::iterator temp_itr = fusions.find(Fusion(fusion.refid2, fusion.refid1, fusion.right, fusion.left, fusion.dir));
	      if (temp_itr != fusions.end())
		{
		  ++(itr->second.unsupport_count);
		  ++(temp_itr->second.unsupport_count);
		}
	...
	case FUSION_RF:
	  ...
	  if (fusion.dir == FUSION_FF ||
	      ...
	      !auto_sort)
	    {
	      ...
	      if (fusion.dir == FUSION_RF)
		fusion.left = positionInGenome + 1;
	      else
		fusion.left = positionInGenome - 1;
	  ...
	  else
	    {
	      ...
	      if (fusion.dir == FUSION_RF)
		fusion.right = positionInGenome - 1;
	      else
		fusion.right = positionInGenome + 1;
	    }
...
void pair_support(const HitsForRead& left_hits, const HitsForRead& right_hits, FusionSet& fusions, FusionSet& fusions_ref)
{
  ...
  for (size_t i = 0; i < left.size(); ++i)
      ...
      if (fusionSpanned)
	{
	  ...
	  fusion_leftSide = lh.ref_id() == rh.ref_id();
	  int temp1, temp2;
	  ...
	  if (dir != FUSION_RF && fusionHit->antisense_align() && (!fusion_leftSide || otherHit->antisense_align()))
	    unsupport = true;

	  if (dir != FUSION_FR && !fusionHit->antisense_align() && (fusion_leftSide || !otherHit->antisense_align()))
	    unsupport = true;
	  ...
	  // daehwan - debug
#if 0
	  // if (fusion.left == 106312009 && fusion.right == 128806979)
	    {
	      cout << "daehwan - test: " << lh.insert_id() << endl;
      ...
      else
	{
	  if (dir == FUSION_FF)
	      ...
	      if (lh.antisense_align())
		{
		  if (rh.right() >= inner_dist)
		    left1 = rh.right() - inner_dist;
		  
		  right2 = lh.left() + inner_dist;
	  ...
	  if ((dir == FUSION_FR || dir == FUSION_RF)
	      && (ref_id1 > ref_id2 || (ref_id1 == ref_id2 && lh.left() > rh.left())))
	    {
	      uint32_t temp = ref_id1;
	      ref_id1 = ref_id2;
	      ref_id2 = temp;
      ...
      while (lb != ub && lb != fusions_ref.end())
	{
	  if (lb->first.dir == dir &&
	      ...
	      if (!fusionSpanned || !fusion_leftSide)
		{
	      ...
	      // if(!fusionSpanned)
	      // if (pass && !unsupport)
	      // if (lb->first.left == 106312009 && lb->first.right == 128806979)
	      const char* dir_str = "ff";
	      if (dir == FUSION_FR)
		dir_str = "fr";
	      else if (dir == FUSION_RF)
