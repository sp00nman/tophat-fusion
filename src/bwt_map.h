...
enum CigarOpCode { CIGAR_NOTHING = 0, FUSION_NOTHING = 0, MATCH, mATCH, INS, iNS, DEL, dEL, FUSION_FF, FUSION_FR, FUSION_RF, REF_SKIP, rEF_SKIP, SOFT_CLIP, HARD_CLIP, PAD };
...
BowtieHit(uint32_t ref_id,
  ...
  CigarOpCode fusion_opcode() const
  {
	...
	if (op.opcode == FUSION_FF || op.opcode == FUSION_FR || op.opcode == FUSION_RF)
	  return op.opcode;
      }
  ...
  bool antisense_align2() const
  {
    ...
    if (fusionOpCode == FUSION_NOTHING || fusionOpCode == FUSION_FF)
...
public:
	...
	inline static uint32_t hash_string(const char* __s)
	{
