...
def filter_fusion(params):
    def filter_fusion_impl(fusion, refGene_list, ensGene_list, ensGtp_dic, seq_chr_dic, fusion_gene_list):
        ...
        for line in fusion_file:
            ...
            both = num_reads + num_pair_ends
            all = both + num_pair_ends_fusion
            ...
            if string.find(data, "maher") != -1:
                if single:
                    ...
                    if num_unsupport_reads > num_reads * 10:
                        continue
                else:
                    if string.find(sample_name, "VCaP") != -1:
                        if num_pair_ends < 1 or all < 10:
                            continue

                        if num_unsupport_reads > both:
                            continue

                    else: # UHR single vs. paired
                        if (both < 3 or num_pair_ends < 2) and all < 10:
                            continue

                        if num_unsupport_reads > basic:
                            continue
            """
            ...
            dir = info[3]
            gene1, gene1_name, gene1_where, gene1_belong, gene1_sense, ens_gene1 = find_gene(chr1, coord1, dir[0], True)
            gene2, gene2_name, gene2_where, gene2_belong, gene2_sense, ens_gene2 = find_gene(chr2, coord2, dir[1], False)

            if ens_gene1 == ens_gene2 or gene1 == "N/A" or gene2 == "N/A" or \
...
def read_dist(params):
    def check_region(sam, check_list):
        output = ''
        ...
        for i in range(len(check_list)):
            ...
            if len(reads_list[i]) <= 0:
                continue

            reads = sorted(reads_list[i], cmp=my_cmp)
    ...
    for line in file:
        if line_no % 6 == 0:
            ...
            if not os.path.exists("%s/%s_%s" % (dir, sample_name, "_".join(temp_list[1:5]))):
    ...
    for sample_name, list in check_list.items():
        ...
        for i in range((len(list) + increment - 1) / increment):
            ...
            def work():
                check_output_list = check_region(sam_file_name, check_list).split('@')
    ...
    def generate_html_impl(fusion_list, cluster_list):
        ...
        for c in range(len(cluster_list)):
            ...
            for i in indices:
                ...
                print >> html_file, r'<TD ALIGN="RIGHT">%d</TD></TR>' % stats[2]
