...
AlignStatus::AlignStatus(const BowtieHit& bh){
	...
	if(cigar.size() > 0){
		...
		_fusionFreeAlignment = 1;
		vector<CigarOp>::const_iterator it;
		for(it = cigar.begin(); it < cigar.end(); ++it){
			if(it->opcode == INS || it->opcode == iNS || it->opcode == DEL || it->opcode == dEL){
				_indelFreeAlignment = 0;
			}
			if(it->opcode == REF_SKIP || it->opcode == rEF_SKIP){
				_spliceFreeAlignment = 0;
			}
			if(it->opcode == FUSION_FF || it->opcode == FUSION_FR || it->opcode == FUSION_RF){
			  _fusionFreeAlignment = 0;
			}
		}
	} 
